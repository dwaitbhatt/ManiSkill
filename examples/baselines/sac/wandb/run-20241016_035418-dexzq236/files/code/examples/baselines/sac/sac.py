...
class ReplayBuffer:
    ...
    def clean_samples(self, obs: torch.Tensor, next_obs: torch.Tensor, action: torch.Tensor, reward: torch.Tensor, done: torch.Tensor):
        ...
        large_mask = (obs > 1e3).any(dim=1) | (next_obs > 1e3).any(dim=1) | (action > 1e3).any(dim=1)
        # Find all row indices where any of the tensors are nan or too large
        ...
        if len(nan_or_large_indices) > 0:
            ...
            # Set these rows from the tensors to zero
...
class Actor(nn.Module):
    ...
    def get_eval_action(self, x):
        x = self.backbone(x)
        mean = self.fc_mean(x)
        nan_mask = torch.isnan(mean).any(axis=1)
        if nan_mask.any():
            mean[nan_mask] = 0
    ...
    def get_action(self, x):
        ...
        nan_mask = torch.isnan(mean).any(axis=1) | torch.isnan(std).any(axis=1)
        if nan_mask.any():
            mean[nan_mask] = 0
            std[nan_mask] = 1
        try:
...
if __name__ == "__main__":
    ...
    # env setup
    env_kwargs = dict(obs_mode="state", control_mode="pd_joint_delta_pos", render_mode="rgb_array", sim_backend="gpu", robot_uids=args.robot)
    ...
    pbar = tqdm(total=args.total_timesteps, desc="Global Step")
