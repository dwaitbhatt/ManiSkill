...
class Actor(nn.Module):
    ...
    def forward(self, obs, detach_encoder=False):
        ...
        action = torch.tanh(self.fc_out(x)) * self.action_scale + self.action_bias
        return action, visual_feature
    ...
    def get_action(self, obs, detach_encoder=False):
        action, visual_feature = self(obs, detach_encoder)
        return action, visual_feature
    ...
    while global_step < args.total_timesteps:
        ...
        for local_step in range(args.steps_per_env):
            ...
            if not learning_has_started:
                ...
                if not args.evaluate:
                    noise = torch.randn_like(actions) * args.exploration_noise  # Adjust noise scale as needed
        ...
        for local_update in range(args.grad_steps_per_iteration):
            ...
            with torch.no_grad():
                ...
                noise = torch.randn_like(data.actions) * args.target_policy_noise
                noise = noise.clamp(-args.noise_clip, args.noise_clip)
                next_actions, visual_feature = actor(data.next_obs)
                ...
                # Get target Q-values
                qf1_next_target = qf1_target(data.next_obs, next_actions, visual_feature)
                qf2_next_target = qf2_target(data.next_obs, next_actions, visual_feature)
            ...
            if global_update % args.policy_frequency == 0:
                # Compute actor loss
                actions, visual_feature = actor(data.obs)
                actor_loss = -qf1(data.obs, actions, visual_feature, detach_encoder=True).mean()
