...
class XArm6XArmGripper(BaseAgent):
    ...
    def _after_loading_articulation(self):
        ...
        finger = self.robot.active_joints_map["right_finger_joint"]
        link_inner_knuckle = inner_knuckle.get_child_link()
        link_finger = finger.get_child_link()
        
        # the next 4 magic arrays were computed using https://github.com/haosulab/cvpr-tutorial-2022/blob/master/debug/robotiq.py
        # for robotiq 2f-85-gripper. 
        # TODO: Current values are not good. Repeat the process for xarmgripper.
        p_f_right = [0.19493, -0.0242176, -0.81818]
        p_p_right = [0.163336, 0.00387235, -0.845391]
        p_f_left = [0.202573, -0.0628078, -0.819182]
        p_p_left = [0.183938, -0.0967702, -0.848354]

        right_drive = self.scene.create_drive(
            link_inner_knuckle, sapien.Pose(p_f_right), link_finger, sapien.Pose(p_p_right)
        )
        ...
        finger = self.robot.active_joints_map["left_finger_joint"]
        link_inner_knuckle = inner_knuckle.get_child_link()
        link_finger = finger.get_child_link()
    
        left_drive = self.scene.create_drive(
            link_inner_knuckle, sapien.Pose(p_f_left), link_finger, sapien.Pose(p_p_left)
        )
