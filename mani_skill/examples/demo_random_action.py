...
class Args:
    ...
    """Directory to save RGBD observations and camera poses when pressing the period key"""
...
def main(args: Args):
    ...
    while True:
        ...
        # Check for period key press to save observation
        if args.render_mode == "human" and isinstance(viewer, sapien.utils.Viewer) and viewer.window.key_press("v"):
            observation_count += 1
            timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
            obs_folder = save_dir / f"observation_{timestamp}_{observation_count}"
            obs_folder.mkdir(exist_ok=True, parents=True)
            
            print(f"Saving RGBD observation to {obs_folder}")
